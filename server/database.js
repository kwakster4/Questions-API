...
mongoose.connect(`mongodb://${process.env.USER1}:${process.env.PWD}@${process.env.HOST}/sdc_q_a`, {useNewUrlParser: true, useUnifiedTopology: true, useCreateIndex: true, useFindAndModify: false});
...
const getAs = function(question_id, page, count) {
  ...
  return Question.aggregate([{$match: {id: question_id}}, {$unwind: '$answers'}, {$match: {'answers.reported': {$ne: 1}}}, {$project: {'answers':1, _id:0}}, {$limit: count}]).option({maxTimeMS: 50, allowDiskUse: true})
    .then((answers)=>{
